...
def _is_full_state_dict(fsdp_config):
    """Check if the checkpoint type is FULL_STATE_DICT (works with both enum and string)."""
    ct = fsdp_config.checkpoint_type
    if isinstance(ct, str):
        return "FULL_STATE_DICT" in ct
    return ct.name == "FULL_STATE_DICT" if hasattr(ct, "name") else False


class CosineWarmupStableLRScheduler:
    """
    Linear warmup followed by cosine decay to a stable minimum LR.
    """
    ...
    def get_lr(self):
        if self.current_step < self.warmup_steps:
            # Linear warmup
            lr_scale = (self.current_step + 1) / max(1, self.warmup_steps)
            ...
            # Stable minimum LR after total_steps
            return self.min_lr
...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None, wandb_run=None):
    ...
    effective_eval_steps = (
        train_config.eval_steps * gradient_accumulation_steps if train_config.eval_steps > 0
        else 0
    )
    
    # Run evaluation before any training
    if train_config.run_validation and eval_dataloader is not None and train_config.run_validation_before_train:
        if not train_config.enable_fsdp or rank == 0:
            print("\n--- Running pre-training evaluation (epoch 0) ---")
        ...
        if not train_config.enable_fsdp or rank == 0:
            print(f"Pre-training eval: ppl={eval_ppl:.4f}, loss={eval_epoch_loss:.4f}")
        ...
        if wandb_run:
            wandb_run.log({"eval/pre_train_perplexity": eval_ppl, "eval/pre_train_loss": eval_epoch_loss})
    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            model.train()
            total_loss_sum = 0.0  # sum of CE across all tokens in epoch
            total_token_count = 0  # total non-ignored tokens in epoch
            total_length = len(train_dataloader)//gradient_accumulation_steps
            pbar = tqdm(colour="blue", desc=f"Training Epoch: {epoch+1}", total=total_length, dynamic_ncols=True)
            with profile(train_config,local_rank) as profile_context:
                accum_token_count = 0  # track tokens across micro-batches for proper normalization
                for step, batch in enumerate(train_dataloader):
                    ...
                    # Count non-ignored tokens for token-level gradient normalization
                    num_tokens_in_batch = (batch["labels"] != -100).sum()
                    accum_token_count += num_tokens_in_batch.item()
                    with autocast():
                        loss = model(**batch).loss
                    _raw_loss = loss.detach().float()
                    total_loss_sum += _raw_loss * num_tokens_in_batch.item()
                    total_token_count += num_tokens_in_batch.item()
                    # Token-weighted loss: backward(loss * num_tokens) accumulates
                    # d(sum_CE)/d(params) instead of d(mean_CE)/d(params).
                    # We normalize by total tokens at the optimizer step.
                    # This makes training invariant to gradient_accumulation_steps.
                    loss_for_backward = loss * num_tokens_in_batch
                    if train_config.save_metrics:
                        train_step_loss.append(_raw_loss.item())
                        train_step_perplexity.append(float(torch.exp(_raw_loss)))
                    if train_config.use_fp16:
                        ...
                        if (step + 1) % gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                            ...
                            raise NotImplementedError(
                                "In the current impl., not using train_config.use_fp16. "
                                "In later version when using this branch, can copy-paste from below."
                            )
                    else:
                        # regular backpropagation when fp16 is not used
                        loss_for_backward.backward()
                        if (step + 1) % gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                            # Normalize accumulated gradients by total tokens
                            # FSDP2 AVG reduce-scatter gives (1/W) * sum_gpu d(sum_CE),
                            # we want d(sum_CE) / total_tokens_all_gpus.
                            # So scale by W / total_tokens_all_gpus.
                            _token_count_tensor = torch.tensor(
                                [accum_token_count], device=torch.cuda.current_device(), dtype=torch.float64
                            )
                            if train_config.enable_fsdp:
                                dist.all_reduce(_token_count_tensor, op=dist.ReduceOp.SUM)
                            total_tokens = _token_count_tensor.item()
                            _grad_scale = (world_size / total_tokens) if train_config.enable_fsdp else (1.0 / total_tokens)
                            for p in model.parameters():
                                if p.grad is not None:
                                    p.grad.mul_(_grad_scale)
                            accum_token_count = 0
                            if train_config.gradient_clipping and train_config.gradient_clipping_threshold > 0.0:
                            ...
                            optimizer.zero_grad()
                            pbar.update(1)
                            # Step-based LR scheduling for custom linear-warmup-cosine-decay
                            if isinstance(lr_scheduler, CosineWarmupStableLRScheduler):
                                lr_scheduler.step()

                        if effective_eval_steps > 0 and total_train_steps % effective_eval_steps == 0:
                            if train_config.run_validation and eval_dataloader is not None:
                                print(f"\n--- Running step-based evaluation at step {total_train_steps} ---")
                                eval_ppl, eval_epoch_loss, temp_val_loss, temp_step_perplexity = evaluation(
                                    model, train_config, eval_dataloader, local_rank, tokenizer, wandb_run
                                )
                                if train_config.save_metrics:
                                ...
                                if eval_epoch_loss < best_val_loss:
                                    best_val_loss = eval_epoch_loss
                                    print(f"New best validation loss: {best_val_loss:.4f} at step {total_train_steps}")
                                # save regardless of validation result
                                if train_config.save_model:
                                    _time_spent = save_checkpoint_at_step(model, optimizer, rank, train_config, fsdp_config, total_train_steps)
                                    if rank==0:
                                        print(f"Time taken for checkpointing at step {total_train_steps}: {_time_spent:.2f} seconds")
                                model.train()
                    ...
                    if wandb_run:
                        if not train_config.enable_fsdp or rank==0:
                            ...
                            current_lr = lr_scheduler.get_lr() if isinstance(lr_scheduler, CosineWarmupStableLRScheduler) else lr_scheduler.get_last_lr()[0]
                            log_dict = {
                                'train/epoch': epoch + 1,
                                'train/step': total_train_steps,
                                'train/loss': _raw_loss,
                                'train/learning_rate': current_lr,
                            }
                            if grad_norm is not None:
                                log_dict['train/grad_norm'] = grad_norm.item() if hasattr(grad_norm, 'item') else float(grad_norm)
                            wandb_run.log(log_dict)

                    pbar.set_description(f"Training Epoch: {epoch+1}/{train_config.num_epochs}, step {step}/{len(train_dataloader)} completed (loss: {_raw_loss})")
                    if train_config.save_metrics:
        ...
        # Reduce total CE sum and token count across all devices
        _epoch_loss_sum = torch.tensor([total_loss_sum], device=torch.cuda.current_device())
        _epoch_token_count = torch.tensor([total_token_count], device=torch.cuda.current_device(), dtype=torch.float64)
        if train_config.enable_fsdp:
            dist.all_reduce(_epoch_loss_sum, op=dist.ReduceOp.SUM)
            dist.all_reduce(_epoch_token_count, op=dist.ReduceOp.SUM)
        train_epoch_loss = _epoch_loss_sum.item() / _epoch_token_count.item()
        ...
        # Update the learning rate as needed (only for epoch-based schedulers)
        # CosineWarmupStableLRScheduler is updated per-step, not per-epoch
        if not isinstance(lr_scheduler, CosineWarmupStableLRScheduler):
            lr_scheduler.step()
        ...
        if should_save_model:
            ...
            if train_config.use_peft:
                ...
                if not train_config.enable_fsdp:
                ...
                elif _is_full_state_dict(fsdp_config):
                    ...
                    # Default: sharded checkpoint (DCP)
                    if train_config.save_optimizer:
...
def evaluation(model,train_config, eval_dataloader, local_rank, tokenizer, wandb_run):
    ...
    val_step_loss = []
    val_step_perplexity = []
    eval_loss_sum = 0.0  # sum of CE across all tokens (not mean per batch)
    eval_token_count = 0  # total non-ignored tokens
    total_eval_steps = 0
    with MemoryTrace() as memtrace:
        for step, batch in enumerate(tqdm(eval_dataloader,colour="green", desc="evaluating Epoch", dynamic_ncols=True)):
            ...
            with torch.no_grad():
                ...
                # Accumulate sum of CE (not mean) for proper per-token averaging
                eval_loss_sum += loss.detach().float() * num_tokens
                eval_token_count += num_tokens
    ...
    # Reduce total CE sum and token count across all devices
    eval_loss_sum = torch.tensor([eval_loss_sum], device=torch.cuda.current_device())
    eval_token_count_t = torch.tensor([eval_token_count], device=torch.cuda.current_device(), dtype=torch.float64)
    if train_config.enable_fsdp:
        dist.all_reduce(eval_loss_sum, op=dist.ReduceOp.SUM)
        dist.all_reduce(eval_token_count_t, op=dist.ReduceOp.SUM)

    # Compute per-token mean loss (invariant to batch size)
    eval_epoch_loss = eval_loss_sum.item() / eval_token_count_t.item()
...
def save_checkpoint_at_step(model, optimizer, rank, train_config, fsdp_config, step):
    """
    Helper function to save checkpoint at a given step
    """
    checkpoint_start_time = time.perf_counter()
    ...
    if train_config.use_peft:
        if train_config.enable_fsdp:
            if rank == 0:
                print(f"save_checkpoint_at_step():: Saving PEFT ckpt at step {step}")
        else:
            print(f"save_checkpoint_at_step():: Saving PEFT ckpt at step {step}")
        save_peft_checkpoint(model, train_config.output_dir)
        ...
        if not train_config.enable_fsdp:
            ...
            print(f"Saving FSDP checkpoint at step {step} using FULL_STATE_DICT")
            save_fsdp_model_checkpoint_full(
                model, optimizer, rank, train_config, epoch=None, step=step
            )
            if train_config.save_optimizer:
                save_optimizer_checkpoint(
                    model, optimizer, rank, train_config, epoch=None, step=step
            ...
            # Default: sharded checkpoint (DCP)
            if train_config.save_optimizer:
                print(f"Saving FSDP checkpoint at step {step} using SHARDED_STATE_DICT")
    ...
    return time.perf_counter() - checkpoint_start_time
