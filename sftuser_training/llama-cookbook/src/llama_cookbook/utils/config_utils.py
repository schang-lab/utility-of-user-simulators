...
def check_fsdp_config(fsdp_config):
    VALID_TYPES = ("SHARDED_STATE_DICT", "FULL_STATE_DICT")
    ct = fsdp_config.checkpoint_type
    if isinstance(ct, str):
        # Normalize: "StateDictType.SHARDED_STATE_DICT" -> "SHARDED_STATE_DICT"
        ct = ct.replace("StateDictType.", "")
        fsdp_config.checkpoint_type = ct
    else:
        # Handle legacy enum values
        ct = ct.name if hasattr(ct, "name") else str(ct)
        fsdp_config.checkpoint_type = ct

    if ct not in VALID_TYPES:
        raise ValueError(f"Invalid checkpoint_type {fsdp_config.checkpoint_type}, must be one of {VALID_TYPES}")
